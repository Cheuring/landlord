...
public class ClientEventListener_CODE_GAME_LANDLORD_ELECT extends ClientEventListener {
    ...
    public void call(Channel channel, String data) {
        ...
        if (nextClientId != User.INSTANCE.getId()) {
            SimplePrinter.printNotice("It's not your turn. Please wait patiently for his/her confirmation !");
            SimplePrinter.printNotice("");
        } else {
            StringBuilder s = new StringBuilder("It's  your turn.Please select your score [0");
            for (int i = highestScore + 1; i <= 3; ++i) {
                s.append("/").append(i);
            }
            s.append("] (enter [exit|e] to exit current room)");
            SimplePrinter.printNotice(s.toString());
            String userInput = SimpleWriter.write(User.INSTANCE.getNickname(), "getScore");
            if (userInput.equalsIgnoreCase("exit") || userInput.equalsIgnoreCase("e")) {
                ...
                if (currentScore > highestScore) {
                    result = MapUtil.newInstance()
                            ...
                            .put("currentLandlordId", User.INSTANCE.getId())
                            .json();
                } else if (roomInfo.containsKey("currentLandlordId")) {
                    result = MapUtil.newInstance()
                            .put("highestScore", highestScore)
                            .put("currentLandlordId", roomInfo.get("currentLandlordId"))
                            .json();
                } else {
                    result = MapUtil.newInstance()
                            .put("highestScore", 0)
